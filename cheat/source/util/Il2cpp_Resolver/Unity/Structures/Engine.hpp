...
namespace Unity
{
	...
	struct Vector3
	{
		...
		// Overload the '+' operator (optional, useful for adding vectors)
		Vector3 operator+(const Vector3& other) const
		{
			return Vector3(x + other.x, y + other.y, z + other.z);
		}
		...
		// Magnitude calculation (useful for distance and normalization)
		...
		{
			float mag = Magnitude();
			if (mag == 0.0f) return Vector3(0, 0, 0);
			return *this / mag;
		}

		// Existing ToVectors method remains unchanged
