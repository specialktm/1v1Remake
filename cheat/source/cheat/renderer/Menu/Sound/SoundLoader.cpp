...
namespace cheat
{
    bool SoundLoader::PlaySound(fs::path& file)
	{
        if (!fs::exists(file))
            return false;
        ...
        a_req req = a_req_create(position, 1.f, 1.0f, 0, 0, 0, 0, 0);

        unsigned char* data_ptr = 0;

        // Create the song
        uint16_t song_id = a_song_create_fs(ctx, file.string().c_str(), "song", 32, 3,
            4096 * 4, &data_ptr);
